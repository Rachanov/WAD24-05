...
const ChatComponent: React.FC = () => {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    if (isWaitingForBot) return; // ปิดการพิมพ์ในขณะรอข้อความจาก Bot
  ...
  const handleSendMessage = () => {
    if (inputValue.trim() && !isWaitingForBot) { // เช็คว่ากำลังรอข้อความจาก Bot อยู่หรือไม่
      const newMessage = { sender: 'User', text: inputValue };
      setMessages((prevMessages) => [...prevMessages, newMessage]);
      setInputValue('');
      setIsWaitingForBot(true); // เปลี่ยนสถานะเป็นรอข้อความจาก Bot
      ...
      setTimeout(() => {
        const botReply = { sender: 'Bot', text: 'ขอบคุณสำหรับข้อความของคุณ!' };
        setMessages((prevMessages) => [...prevMessages, botReply]);
        setIsWaitingForBot(false); // ให้พิมพ์ข้อความได้อีกครั้งหลังจาก Bot ตอบกลับ
      }, 3000);
    }
  };
  ...
  return (
    <div className={`flex h-screen antialiased ${theme === 'dark' ? 'bg-zinc-700 text-white' : 'bg-white text-gray-800'}`}>
      ...
      <div className={`flex flex-row h-full w-full overflow-x-hidden`}>
        ...
        {/* chatsection */}
        <div className={`flex flex-col flex-auto h-full p-6 ml-24 transition-all duration-300 ease-in-out ${isSidebarOpen ? "ml-64" : "ml-12"}`}>
          {/* top message */}

          <div className={`fixed flex flex-row items-center h-14 ${isSidebarOpen ? 'max-w-[82.5%]' : 'max-w-[92%]'} mb-0 justify-between navbar top-0 z-50 bg-white ${theme === 'dark' ? 'bg-zinc-900 text-white' : 'bg-white'}`}>
            <Link href={"/"}>
                  ...
                  {messages.map((message, index) => (
                    ...
                    >
                      ...
                      >
                        ...
                        {message.sender !== 'User' && (
                          <img
                            src="/path/to/icon.png" // Replace with the actual path to your icon
                            alt="icon"
