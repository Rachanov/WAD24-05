...
def prepare_model():
    ...
    model = AutoModelForCausalLM.from_pretrained(
        "meta-llama/Llama-3.1-70B-Instruct",  # หรือเลือก size อื่นตามความเหมาะสม
...
def main():
    try:
        # เพิ่มโค้ดนี้ที่จุดเริ่มต้นของ main()
        from huggingface_hub import login
        login(token="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")  # ใส่ token ของคุณ
        
        print("Preparing model and tokenizer...")
        # 1. เตรียม model และ tokenizer
        ...
        print("Testing model...")
        test_question = "วิธีการเลือกโดรนทีเหมาะที่สุดคืออะไร?"
